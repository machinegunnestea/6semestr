...
app
        ...
        if(id) {
            db.serialize(function() {
                db.get("SELECT * FROM Movies WHERE Id = ?;", id, (err, row) =>{
                    ...
                    if (row === undefined) {
                        response.status(404).send(`s with 'id' = ${id} not found.`);
                        return;                     
                    }
    ...
    .post((request, response) =>{
        ...
        if(isMovieValid(movie)){
            db.serialize(function() {
                db.get("SELECT COUNT(*) AS 'Count' FROM Movies WHERE Id = ?;", movie.id, (err, row) => {
                    if (err) {
                        response.status(500).send(err.message);
                        return;
                    }
                    ...
                    db.run("INSERT INTO Movies VALUES(?, ?, ?, ?, ?);", [ movie.id, movie.name, movie.price, movie.description, movie.genre ], (err) => {
                        if (err) {
                            response.status(500).send(err.message);
                            return;
                        }
        ...
        if(isMovieValid(movie)){
            db.serialize(function() {
                db.get("SELECT COUNT(*) AS 'Count' FROM Movies WHERE Id = ?;", movie.id, (err, row) => {
                    ...
                    if (row.Count == 0) {
                        response.status(404).send(`Movie with 'id' = ${movie.id} is not found in the database.`);
                        return;
                    }

                    db.run("UPDATE Movies SET Name = ?, Price = ?, Description = ?, Genre = ? WHERE Id = ?;", 
    ...
    .delete((request, response) =>{
        ...
        if(id){
            db.serialize(function() {
                db.get("SELECT COUNT(*) AS 'Count' FROM Movies WHERE Id = ?;", id, (err, row) => {
                    if (err) {
                        response.status(500).send(err.message);
                        return;
                    }
                    ...
                    if (row.Count == 0) {
                        response.status(404).send(`Movie with 'id' = ${id} is not found in the database.`);
                        return;
                    }
                    ...
                    db.run("DELETE FROM Movies WHERE Id = ?;", id, (err) => {
                        ...
                        response.status(200).send(`Movie with 'id' = ${id} was successfully deleted.`);
                    });
                });
            });
        }
